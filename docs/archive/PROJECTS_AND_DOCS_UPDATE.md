> **Archive:** Snapshot from 2024-11. The `docs/system/`, `docs/server/`, and `docs/reference/` trees described below were removed in a later cleanup. **Migration file location today:** [docs/database/migrations/001_create_projects_table.sql](../database/migrations/001_create_projects_table.sql). Regenerate `src/database.types.ts` with your Supabase (or other) codegen when the schema changes.
...
**File:** `docs/database/migrations/001_create_projects_table.sql`
...
*(Historical — not all paths exist in the repo today.)*
...
```sql
-- Execute docs/database/migrations/001_create_projects_table.sql in Supabase SQL Editor
```

### 2. Update Database Types

Regenerate `src/database.types.ts` using your Supabase project (e.g. Supabase CLI `gen types`).
...
**See:** `docs/reference/CHANGELOG.md` for full details *(file removed from repo; see archive note above).*
