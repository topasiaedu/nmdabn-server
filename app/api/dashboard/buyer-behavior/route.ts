...
/**
 * GET /api/dashboard/buyer-behavior — Buyer Behavior dashboard sections via get_buyer_behavior_stats RPC.
 */
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const { data, error } = await supabase.rpc("get_buyer_behavior_stats", {
    ...
    if (error !== null) {
      ...
      return NextResponse.json(
        { success: false, error: "Failed to load buyer behavior stats" },
        { status: 500 }
      );
    }
