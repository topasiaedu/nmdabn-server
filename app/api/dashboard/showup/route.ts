...
/**
 * GET /api/dashboard/showup — Show Up dashboard stats (NM / OM / MISSING buckets) via get_showup_stats RPC.
 */
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const { data, error } = await supabase.rpc("get_showup_stats", {
    ...
    if (error !== null) {
      ...
      return NextResponse.json(
        { success: false, error: "Failed to load show up stats" },
        { status: 500 }
      );
    }
